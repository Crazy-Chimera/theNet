...
def test_phi_derived_allocation_uses_structural_coherence():
    utility = create_relational_utility("agent:a", 1.0, ["evidence"], True, STAMP)
    ...
    result = allocate_memory_and_compute_from_phi(
        [utility], memory, compute, {"agent:a": make_structure("a", "b")}
    )
...
def test_phi_derived_allocation_rejects_missing_structure():
    ...
    try:
        allocate_memory_and_compute_from_phi([utility], memory, compute, {})
